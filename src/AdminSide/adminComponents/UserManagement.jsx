...
export function UserManagement() {
  ...
  let [allAdminData, setAllAdminData] = useState([]);
  ...
  let [adminData,setAdminData] = useState(false)
  ...
  async function getAllBlockedUserData() {
   ...
   async function getAllAdminData() {
    try {
      let adminData = await axios.get(`${baseUrl}/admin/`);

      setAllAdminData(adminData.data);
    } catch (err) {
      alert(err);
    }
  }
  ...
  async function deleteUser(user) {
    try {
      ...
      if (deleteUser.status == 200) {
        ...
        console.log(addToBlock,"current")
  ...
  return (
    <Box>
      ...
      >
        ...
        >
          {showBlockData ? "Hide Admin" : "Admins Data"}
        </Button>
