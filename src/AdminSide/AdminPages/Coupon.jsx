...
export function Coupon() {
  ...
  async function postCouponData() {
    try {
      let couponData = await axios.post(`${baseUrl}/coupon/add`, localFormData, {headers});
...
let deleteCoupon = async(id)=>{
    try{
      ...
      if (userResponse) {
        let deleteData = await axios.delete(`${baseUrl}/coupon/delete/${id}`, {headers})
...
let updateCoupon = async(e)=>{
    e.preventDefault()
    try{
        let updateData = await axios.patch(`${baseUrl}/coupon/patch/${updateValues._id}`,updateValues, {headers})
